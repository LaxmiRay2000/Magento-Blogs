 ...
 */
if ( ! function_exists( 'book_author_blog_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function book_author_blog_posted_on($icon = false) {
...
if ( ! function_exists( 'book_author_blog_time' ) ) {
	function book_author_blog_time() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
...
function book_author_blog_reading_time() {
	// load the content
...
if ( ! function_exists( 'book_author_blog_posted_by' ) ) :
	function book_author_blog_posted_by( $author_image = true ) {
		...
		if ( false === $author_image ) {
			$get_author_image = __( 'Posted by', 'book-author-blog' );
		}
		$postedBy = sprintf(
			$posted_by_format,
...
if ( ! function_exists( 'book_author_blog_comment_popuplink' ) ) {
	function book_author_blog_comment_popuplink() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				__( 'Post a Comment', 'book-author-blog' ),
				__( '1 Comment', 'book-author-blog' ),
				__( '% Comments', 'book-author-blog' ),
				$css_class,
				__( 'Comments are Closed', 'book-author-blog' )
			);
			echo '</span>';
		}
	}
}
function book_author_blog_categories() {
...
if ( ! function_exists( 'book_author_blog_post_tag' ) ) {
	function book_author_blog_post_tag() {
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
...
if ( ! function_exists( 'book_author_blog_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function book_author_blog_post_thumbnail() {
		...
		if ( is_single() || is_page() ) {
			 the_post_thumbnail( 'book-author-blog-thumbnail-large' );
		} else {
...
/**
 * Book Author Blog Navigation
 */
function book_author_blog_navigation() {
...
function book_author_blog_post_page_navigation(){
	if (get_next_post_link() || get_previous_post_link()):
		?>
		<div class="d-flex single-post-navigation justify-content-between">
			<?php if (get_previous_post_link()): ?>
				<div class="previous-post">
					<div class="postarrow"><i class="fa fa-long-arrow-left"></i><?php echo esc_html_e( 'Previous Post', 'book-author-blog' ); ?></div>
			...
			?>
			<div class="next-post">
				<div class="postarrow"><?php echo esc_html_e( 'Next Post', 'book-author-blog' ); ?><i class="fa fa-long-arrow-right"></i></div>
...
function book_author_blog_postedby(){
	?>
...
function book_author_blog_theme_by(){
	?>
	<div class="theme-by-wrapper">
		<div class="theme-by-inner">
			<strong><?php esc_html_e( 'Theme By: ', 'book-author-blog' );?></strong><a href="<?php echo esc_url('https://rswpthemes.com');?>"><?php esc_html_e( 'rswpthemes.com', 'book-author-blog' );?></a>
		</div>
	</div>
	<?php
}