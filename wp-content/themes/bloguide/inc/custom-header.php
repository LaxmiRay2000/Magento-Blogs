 ...
 *
 * @uses bloguide_header_style()
 */
function bloguide_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'bloguide_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'bloguide_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'bloguide' ),
		),
	) );
}
add_action( 'after_setup_theme', 'bloguide_custom_header_setup' );

if ( ! function_exists( 'bloguide_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see bloguide_custom_header_setup().
	 */
	function bloguide_header_style() {
		...
		wp_add_inline_style( 'bloguide-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'bloguide_header_style', 10 );